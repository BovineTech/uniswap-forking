...
const CustomAutoRow = styled(AutoRow)<object>`
  @media (min-width: ${({ theme }) => `${theme.breakpoint.md}px`}) {
    flex-wrap: nowrap;
  }
`
const SubTitle = styled(ThemedText.DeprecatedLabel)<object>`
  flex: 45%;
  padding-right: 10px;
`
...
function AddLiquidity() {
  ...
  const Buttons = () =>
    addIsUnsupported ? (
      ...
      <ButtonLight onClick={toggleWalletDrawer} padding="12px">
        <Trans>Connect wallet</Trans>
      </ButtonLight>
    ) : (
      <CustomAutoRow gap="8px">
                      ...
                      <>
                        {Boolean(price && baseCurrency && quoteCurrency && !noLiquidity) && (
                          <AutoColumn gap="2px" style={{ marginTop: '0.5rem' }}>
                            <Trans>
                              <ThemedText.DeprecatedMain fontWeight={535} fontSize={12}>
                                Current price:
                              </ThemedText.DeprecatedMain>
