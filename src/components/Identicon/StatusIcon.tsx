...
export const IconWrapper = styled.div<{ size?: number; hideShowMobile?: boolean }>`
  position: relative;
  ${flexColumnNoWrap};
  align-items: center;
  justify-content: center;
  margin-right: 4px;
  display: ${({ hideShowMobile }) => (hideShowMobile ? 'none' : 'flex')};
  @media only screen and (max-width: ${navSearchInputVisibleSize}px) {
    display: ${({ hideShowMobile }) => (hideShowMobile ? 'flex' : 'flex')};
  }
  & > img,
  span {
...
export default function StatusIcon({
  ...
  hideShowMobile?: boolean
}) {
  const hasSocks = useHasSocks()

  return (
    <IconWrapper hideShowMobile={hideShowMobile} size={size} data-testid="StatusIconRoot">
