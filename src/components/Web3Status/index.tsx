...
const Web3StatusConnected = styled(Web3StatusGeneric)<{
  ...
  border: 0px solid ${({ pending, theme }) => (pending ? theme.accent1 : theme.surface1)};
  color: ${({ pending, theme }) => (pending ? theme.white : theme.neutral1)};
  border: ${({ isClaimAvailable }) => isClaimAvailable && `0px solid ${colors.purple300}`};
  :hover,
  :focus {
    border: 0px solid ${({ theme }) => theme.surface2};
    // background-color: ${({ pending, theme }) => (pending ? theme.accent2 : theme.surface2)};

    :focus {
      border: 0px solid ${({ pending, theme }) => (pending ? darken(0.1, theme.accent1) : darken(0.1, theme.surface3))};
    }
  }
...
function Web3StatusInner() {
  ...
  if (account) {
    return (
      ...
      >
        {!hasPendingActivity && (
          <StatusIcon hideShowMobile account={account} size={40} connection={connection} showMiniIcons={false} />
        )}
        {hasPendingActivity ? (
          <RowBetween>
            <Text>
          ...
          <AddressAndChevronContainer>
            <div className="header__address_container">
