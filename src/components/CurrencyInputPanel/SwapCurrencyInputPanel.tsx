...
const InputPanel = styled.div<{ hideInput?: boolean }>`
  ${flexColumnNoWrap};
  position: relative;
  border-radius: ${({ hideInput }) => (hideInput ? '6px' : '20px')};
  z-index: 1;
...
const CurrencySelect = styled(ButtonGray)<{
  ...
  align-items: center;
  background-color: ${({ selected, theme }) => (selected ? theme.surface3 : theme.accent3)};
  ...
  outline: none;
  user-select: none;
  border: 0px solid ${({ selected, theme }) => (selected ? theme.surface3 : theme.accent1)};
  font-size: 24px;
  font-weight: 485;
  width: ${({ hideInput }) => (hideInput ? '100%' : 'initial')};
  ...
  &:hover,
  &:active {
    background-color: ${({ theme, selected }) => (selected ? theme.surface1 : theme.accent1)};
  }
    ...
    return (
      <InputPanel hideInput={hideInput} {...rest}>
        ...
        <Container hideInput={hideInput}>
          <ThemedText.SubHeaderSmall style={{ userSelect: 'none', paddingBottom: '4px' }}>{label}</ThemedText.SubHeaderSmall>
