...
const CloseIcon = styled(ChevronsRight).attrs({ size: 28 })`
  stroke: ${({ theme }) => theme.neutral2};
  transform: rotate(180deg);
`

const CloseIconX = styled(X).attrs({ size: 28 })`
  stroke: ${({ theme }) => theme.neutral2};
  transform: rotate(180deg);
  position: absolute;
  right: 16px;
...
const CloseDrawer = styled.div`
  ...
  padding: 28px calc(8px + ${DRAWER_OFFSET}) 28px 18px;
  position: absolute;
  top: 0;
  left: 0;
  z-index: 1;
...
function AccountDrawer() {
  ...
  const { cancelActivation, activationState } = useActivationState()
    ...
    <Container>
      {walletDrawerOpen && (
        <CloseDrawer
          onClick={() => {
            toggleWalletDrawer()
            activationState.status === ActivationStatus.ERROR ? cancelActivation() : null
          }}
